...
> 本文檔是給 AI Agent 看的操作守則。如果是人類維護者，請優先閱讀專案根目錄的 `README.md`、`ARCHITECTURE.md`、`DEPLOYMENT.md` 和 `CMS_GUIDE.md`。
...
#### 教訓 6：KV Namespace 絕對隔離（2026-05-01 慘案）
S2C 相關專案在開發過程中，將「新鮮餸菜包 / SEN 青年就業」等盆菜專案 CMS 資料**直接寫入 CWMNG 的生產環境 KV**，導致 CWMNG 官網首頁顯示完全錯誤的內容。這是極其嚴重的生產事故。

**根因分析**：
1. 開發 session 在處理新專案時，**直接對 CWMNG 的 Worker API (`cwmng-cms-worker...`) 發送了 POST `/api/cms/data`**。
2. 沒有為新專案建立獨立的 KV namespace，而是「順手」複用了 CWMNG 的現成 API。
3. Admin 後台代碼被複製時未修改 `API_BASE_URL`，導致儲存操作命中了錯誤的 Worker。

**強制規定**：
- **每個專案必須擁有獨立的 KV namespace**，絕對禁止跨專案共用 `id = "..."`。
- 建立新專案時，第一步必須是 `wrangler kv namespace create {專案名}`，取得新的獨立 ID。
- 複製 Admin 後台代碼時，**必須同時修改 `API_BASE_URL`**，並在儲存測試前驗證 `curl API_BASE_URL/` 回傳的服務名稱是否正確。
- 任何涉及 CMS 的 POST/PUT 操作前，必須 double-check：這個 URL 屬於當前專案嗎？
...
| 跨專案共用 KV namespace | 其他專案資料覆蓋生產環境 CMS | 每個專案獨立 KV，`wrangler kv namespace create` |
